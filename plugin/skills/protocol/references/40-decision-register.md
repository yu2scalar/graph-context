...
| D28 | Issues become graph nodes (retroactively, via an append-only issue registry and `add-node`); decisions link to them with `resolves` (user decision 2026-09-24; implementation planned, see private plan design-shift) | — | decided, not yet implemented |
| D29 | Decision content lives in one append-only file per decision; nodes carry only `source_ref`; a generated `CURRENT.md` lists the active decisions (the current specification); this register becomes a generated table | refines D5 | decided, not yet implemented |
| D30 | No authored handover: `current_node` is the handover, `/graph:hydrate` output is the view, `/graph:handover` becomes a completion gate; work-in-progress state is recorded as an issue, not prose | supersedes D19, D25 when implemented | decided, not yet implemented |
| D31 | Fold hides instead of deleting: folded decisions/issues stay as nodes under the survivor (`supersedes` / `resolves` edges, status `folded`), hidden from default hydrate output | supersedes D7 when implemented | decided, not yet implemented |
| D32 | Storage stays files (`dependency_graph.json` + Markdown) as the source of truth; a later Graph DB migration is kept mechanical by four conditions: every write through `graph_tool.py`, permanent ids, fixed headings in decision/issue files, the operations log kept; the first step would be a rebuildable read-only DB index generated from the files | — | decided |
| D33 | Backlog model (resolves U26): problems/questions = `issue` nodes; not-yet-started work = `feature`/`function` nodes with `wip_status: PLANNED`; `next` flag on both; the Backlog is a graph-wide *view* (open issues OR PLANNED/IN_PROGRESS/BLOCKED OR next), its filter selectable (config default + per-call options) and whatever it excludes printed as a count per component | resolves U26 | decided, not yet implemented |
| D34 | Working rules: R-a record, then ask — a question to the user is asked only after the node it concerns is written, citing the node id, and the answer updates that node; R-b plans are nodes — every not-yet-started step is a PLANNED node `part_of` its feature, ordered by `depends_on`, the next one flagged `next` | — | decided, not yet implemented |
| D35 | Issue state is stored on the issue node: `issue_status` open | resolved | transferred, `closed_by` (decision id, commit hash or short action text) required when not open, optional `owner` (user | claude) and `trigger`; a decision's `resolves` target must have `issue_status: resolved` (checked by R1). Reason: many issues close by an action (e.g. U20 by a push) or are transferred (U3/U4/U8), so `resolves` alone cannot express the state | resolves U33 | active (3.3.0-dev.1) |
